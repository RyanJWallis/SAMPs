...
library("matrixStats")
Palbo_Pos_Med <- colMedians(as.matrix(Palbo_Pos[,-42]))
Palbo_Neg_Med<- colMedians(as.matrix(Palbo_Neg[,-42]))
Control_Pos_Med <- colMedians(as.matrix(Control_Pos[,-42]))
Control_Neg_Med<- colMedians(as.matrix(Control_Neg[,-42]))


Meds <- rbind(Palbo_Pos_Med,Palbo_Neg_Med,Control_Pos_Med,Control_Neg_Med)

rownames(Meds) <- c("Palbo +ve", "Palbo -ve", "Control +ve", "Control -ve")
colnames <- colnames(Palbo_Pos)
