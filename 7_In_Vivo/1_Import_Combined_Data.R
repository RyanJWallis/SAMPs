...
Palbo_1 <- Palbo_1 %>%
  mutate(Detail = case_when(Class == "Positive" ~ "Palbo", Class == "Negative" ~ "Palbo" ))
Palbo_2 <- Palbo_2 %>%
  mutate(Detail = case_when(Class == "Positive" ~ "Palbo", Class == "Negative" ~ "Palbo" ))
Palbo_3 <- Palbo_3 %>%
  mutate(Detail = case_when(Class == "Positive" ~ "Palbo", Class == "Negative" ~ "Palbo" ))
...
Control_1 <- Control_1 %>%
  mutate(Detail = case_when(Class == "Positive" ~ "Control", Class == "Negative" ~ "Control" ))
Control_2 <- Control_2 %>%
  mutate(Detail = case_when(Class == "Positive" ~ "Control", Class == "Negative" ~ "Control" ))
Control_3 <- Control_3 %>%
  mutate(Detail = case_when(Class == "Positive" ~ "Control", Class == "Negative" ~ "Control" ))
